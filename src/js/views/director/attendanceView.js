...
/**
 * 6. ?먯옣 異쒓껐 醫낇빀 愿由?(renderDirectorAttendance)
...
export function renderDirectorAttendance(container) {
    ...
    const render = () => {
        ...
        if (selectedStudentId) {
            ...
            const classDays = studentClasses.map(c => c.dayOfWeek); // e.g. ['??, '??]
            ...
            while (dateCursor <= end) {
                ...
                if (classDays.includes(cursorDayKo)) {
                    ...
                    studentAttendanceList.push({
                        ...
                        teacherName: teacher ? teacher.name : '誘몄???,
                        status: status,
                        checkTime: time,
                        note: note
                    });
            ...
            <div class="glass-card" style="padding: 1.5rem; margin-bottom: 24px;">
                ...
                <div style="display: flex; gap: 12px; border-bottom: 1px solid var(--border-color); padding-bottom: 12px; margin-bottom: 20px;">
                    <button class="btn ${activeSubTab === 'daily' ? 'btn-primary' : 'btn-none'}" id="tab-btn-daily" style="font-weight: 600;">
                        <i class="fa-solid fa-calendar-day" style="margin-right: 6px;"></i>?쇱옄蹂?異쒓껐 議고쉶
                    </button>
                    <button class="btn ${activeSubTab === 'student' ? 'btn-primary' : 'btn-none'}" id="tab-btn-student" style="font-weight: 600;">
                        <i class="fa-solid fa-user-graduate" style="margin-right: 6px;"></i>?먯깮蹂?異쒓껐 議고쉶
                    </button>
                </div>
                ...
                <div id="subtab-content-daily" style="display: ${activeSubTab === 'daily' ? 'block' : 'none'};">
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 16px;">
                        <div style="display: flex; align-items: center; gap: 12px;">
                            <label for="daily-date-picker" style="font-weight: 600; font-size: 0.95rem; color: var(--text-muted);">議고쉶 ?쇱옄 ?좏깮:</label>
                        ...
                        <div style="font-size: 0.9rem; color: var(--text-muted); font-weight: 500;">
                            ?좏깮???붿씪: <span style="color: var(--primary); font-weight: 700;">${dayOfWeekKo}?붿씪</span> | 珥??섏뾽 ?덉젙: <span style="color: var(--accent); font-weight: 700;">${dailyClasses.length}嫄?/span>
                        </div>
                    </div>
                    ...
                    <div class="table-wrapper">
                        ${
                            dailyClasses.length === 0
                                ? `<div style="text-align: center; color: var(--text-muted); padding: 3rem;">?좏깮?섏떊 ?좎쭨(${selectedDailyDate}, ${dayOfWeekKo}?붿씪)?먮뒗 ?덉젙???섏뾽???놁뒿?덈떎.</div>`
                                : `
                                <table class="custom-table">
                                    ...
                                    <tbody>
                                        ${dailyClasses.map(c => {
                                            let statusBadge = `<span class="badge badge-info">?섏뾽 ?湲?/span>`;
                                            let timeText = '-';
                                            let noteText = c.attendance && c.attendance.note ? c.attendance.note : '-';
                                            
                                            if (c.attendance) {
                                                if (c.attendance.status === 'present') {
                                                    statusBadge = `<span class="badge badge-success">?깆썝</span>`;
                                                    timeText = c.attendance.time || '-';
                                                } else if (c.attendance.status === 'late') {
                                                    statusBadge = `<span class="badge badge-warning">吏媛?/span>`;
                                                    timeText = c.attendance.time || '-';
                                                } else if (c.attendance.status === 'absent') {
                                                    statusBadge = `<span class="badge badge-danger">寃곗꽍</span>`;
                                                    timeText = '-';
                                                }
                                            }
                                            return `
                                                <tr>
                                                    ...
                                                    <td>${c.teacher ? c.teacher.name : '誘몄???}</td>
                                                    <td>${statusBadge}</td>
                ...
                <div id="subtab-content-student" style="display: ${activeSubTab === 'student' ? 'block' : 'none'};">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin-bottom: 1.5rem; align-items: flex-end;">
                        <div>
                            <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 0.85rem; color: var(--text-muted);">?먯깮 ?좏깮</label>
                            <div class="custom-dropdown" id="student-selector-dropdown" style="position: relative; width: 100%;">
                                <div class="custom-dropdown-trigger form-control" style="cursor: pointer; display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; height: 38px; background: rgba(0,0,0,0.02); border: 1px solid var(--border-color); border-radius: var(--radius-md);">
                                    <span id="selected-student-display">${(() => {
                                        ...
                                        return selected ? `${selected.name} (${selected.instrument})` : '?먯깮???좏깮?섏꽭??;
                                    })()}</span>
                                ...
                                <div class="custom-dropdown-menu glass-card" style="display: none; position: absolute; top: 105%; left: 0; width: 100%; z-index: 1000; padding: 10px; box-shadow: 0 10px 25px rgba(0,0,0,0.25); max-height: 250px; overflow-y: auto; background: #ffffff; border: 1px solid var(--border-color); border-radius: var(--radius-md); flex-direction: column; gap: 4px;">
                                    <div style="position: relative; width: 100%; margin-bottom: 8px;">
                                        <input type="text" id="student-search-input" placeholder="?먯깮紐?寃??(珥덉꽦 吏??" class="form-control" style="padding: 6px 10px; padding-left: 30px; font-size: 0.85rem; height: 34px; margin-bottom: 0;">
                        ...
                        <div>
                            <label for="start-date-picker" style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 0.85rem; color: var(--text-muted);">議고쉶 ?쒖옉??/label>
                            <input type="date" id="start-date-picker" value="${filterStartDate}" class="form-control" style="width: 100%; padding: 8px 12px;">
                        </div>
                        <div>
                            <label for="end-date-picker" style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 0.85rem; color: var(--text-muted);">議고쉶 醫낅즺??/label>
                    ...
                    ${
                        !selectedStudentId 
                            ? `<div style="text-align: center; color: var(--text-muted); padding: 2rem;">?먯깮???좏깮?댁＜?몄슂.</div>`
                            : `
                            <!-- Stats Cards -->
                            <div class="metrics-grid" style="margin-bottom: 20px;">
                                <div class="glass-card metric-card" style="padding: 12px 16px;">
                                    ...
                                    <div class="metric-info">
                                        <span class="metric-label" style="font-size: 0.75rem;">珥??섏뾽 ?쇱닔</span>
                                        <span class="metric-value" style="font-size: 1.1rem;">${studentStats.total}??/span>
                                    </div>
                                </div>
                                <div class="glass-card metric-card" style="padding: 12px 16px;">
                                    ...
                                    <div class="metric-info">
                                        <span class="metric-label" style="font-size: 0.75rem;">異쒖꽍瑜?(?깆썝+吏媛?</span>
                                ...
                                <div class="glass-card metric-card" style="padding: 12px 16px;">
                                    ...
                                    <div class="metric-info">
                                        <span class="metric-label" style="font-size: 0.75rem;">吏媛??잛닔</span>
                                        <span class="metric-value" style="font-size: 1.1rem; color: var(--warning);">${studentStats.late}??/span>
                                    </div>
                                </div>
                                <div class="glass-card metric-card" style="padding: 12px 16px;">
                                    ...
                                    <div class="metric-info">
                                        <span class="metric-label" style="font-size: 0.75rem;">寃곗꽍 / ?섏뾽?湲?/span>
                                        <span class="metric-value" style="font-size: 1.1rem; color: var(--danger);">${studentStats.absent} / ${studentStats.pending}??/span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="table-wrapper">
                                ${
                                    studentAttendanceList.length === 0
                                        ? `<div style="text-align: center; color: var(--text-muted); padding: 3rem;">吏?뺣맂 湲곌컙 ?숈븞???덉젙???섏뾽???놁뒿?덈떎.</div>`
                                        : `
                                        <table class="custom-table">
                                            <thead>
                                                <tr>
                                            ...
                                            <tbody>
                                                ${studentAttendanceList.map(item => {
                                                    let badge = `<span class="badge badge-info">?섏뾽 ?湲?/span>`;
                                                    let timeText = '-';
                                                    if (item.status === 'present') {
                                                        badge = `<span class="badge badge-success">?깆썝</span>`;
                                                        timeText = item.checkTime;
                                                    } else if (item.status === 'late') {
                                                        badge = `<span class="badge badge-warning">吏媛?/span>`;
                                                        timeText = item.checkTime;
                                                    } else if (item.status === 'absent') {
                                                        badge = `<span class="badge badge-danger">寃곗꽍</span>`;
                                                    }
        ...
        const filterOptions = (query) => {
            ...
            const isChosungOnly = /^[????+$/.test(cleanQuery);
            ...
            if (filtered.length === 0) {
                optionsList.innerHTML = `<div style="padding: 12px; text-align: center; color: var(--text-muted); font-size: 0.85rem; font-style: italic;">?쇱튂?섎뒗 ?먯깮???놁뒿?덈떎.</div>`;
            } else {
...
/**
 * 7. ?쒕툝由?異쒓껐 ?ㅼ삤?ㅽ겕 (renderKioskAttendance)
 ...
 * and choose between check-in (?깆썝) or check-out (?섏썝).
 */
export function renderKioskAttendance(container) {
    ...
    const getKioskPassword = () => {
        ...
        if (!currentUser || !currentUser.academyId) return '';

        const academy = stateStore.getAcademy(currentUser.academyId);
        return academy?.tabletPassword || '';
    };
        ...
        if (key === 'clear') {
            ...
            if (inputDigits.length === 4) {
                ...
                if (matched.length > 0) {
                    ...
                    if (displayMsg) {
                        displayMsg.textContent = '?쇱튂?섎뒗 ?먯깮???놁뒿?덈떎. 踰덊샇瑜??ㅼ떆 ?뺤씤?댁＜?몄슂.';
        ...
        if (key === 'clear') {
            ...
            if (adminAuthDigits.length === 4) {
                if (adminAuthDigits === getKioskPassword()) {
                    ...
                    if (displayMsg) {
                        displayMsg.textContent = '鍮꾨?踰덊샇媛 ?쇱튂?섏? ?딆뒿?덈떎. ?ㅼ떆 ?낅젰?댁＜?몄슂.';
                        displayMsg.style.color = 'var(--danger)';
                    }
                    const dots = container.querySelectorAll('.kiosk-pin-dot');
                    dots.forEach(d => {
    ...
    const triggerCheckIn = (studentId) => {
        ...
        stateStore.markAttendance(studentId, todayStr, 'present', nowTimeStr, '?쒕툝由??깆썝 ?먮룞 ?낅젰');
        ...
        if (!container.querySelector('.kiosk-layout-container')) {
            container.innerHTML = `
                <button class="btn-kiosk-return" id="kiosk-return-to-admin">
                    <i class="fa-solid fa-arrow-right-from-bracket"></i> 愿由ъ옄 紐⑤뱶濡??뚯븘媛湲?
                </button>
        ...
        if (activeStep === 'keypad') {
            kioskHtml = `
                <div style="text-align: center; margin-bottom: 2rem;">
                    ...
                    <h2 style="font-size: 1.8rem; font-weight: 800; margin-bottom: 8px;">${settings.academyName || '?쒕쭅 ?뚯븙?숈썝'} 異쒓껐 ?ㅼ삤?ㅽ겕</h2>
                    <p id="kiosk-message-banner" style="color: var(--text-muted); font-size: 0.95rem;">?대???踰덊샇 ?룹옄由?4?먮━瑜??곗튂??二쇱꽭??</p>
                </div>
                ...
                <div class="kiosk-keypad">
                    ...
                    `).join('')}
                    <button class="kiosk-key key-clear" data-key="clear">?꾩껜吏?</button>
            ...
            kioskHtml = `
                <div style="text-align: center; margin-bottom: 2rem;">
                    ...
                    <h2 style="font-size: 1.8rem; font-weight: 800; margin-bottom: 8px;">愿由ъ옄 蹂몄씤 ?몄쬆</h2>
                    <p id="kiosk-admin-message-banner" style="color: var(--text-muted); font-size: 0.95rem;">鍮꾨?踰덊샇 4?먮━瑜??낅젰?댁＜?몄슂.</p>
                </div>
                ...
                <div style="text-align: center; margin-top: 2rem;">
                    <button class="btn btn-none" id="kiosk-admin-cancel" style="padding: 10px 24px; font-weight: 600; color: var(--text-muted);">
                        <i class="fa-solid fa-xmark" style="margin-right: 8px;"></i> 異쒓껐 ?붾㈃?쇰줈 ?뚯븘媛湲?
                    </button>
                </div>
            `;
            ...
            kioskHtml = `
                <div style="text-align: center; margin-bottom: 2rem;">
                    ...
                    <h2 style="font-size: 1.6rem; font-weight: 800; margin-bottom: 8px;">?먯깮 ?대쫫 ?좏깮</h2>
                    <p style="color: var(--text-muted); font-size: 0.95rem;">蹂몄씤???대쫫???곗튂??二쇱꽭??</p>
                </div>
                ...
                <div class="kiosk-student-grid">
                    ${matchedStudents.map(student => `
                        <div class="kiosk-student-card" data-student-id="${student.id}">
                            ...
                            <div class="kiosk-student-desc">${student.instrument} / ${student.school || '?숈썝??}</div>
                        </div>
                    `).join('')}
                </div>
                ...
                <div style="text-align: center; margin-top: 2rem;">
                    <button class="btn btn-none" id="kiosk-back-to-keypad" style="padding: 10px 24px; font-weight: 600; color: var(--text-muted);">
                        <i class="fa-solid fa-arrow-left" style="margin-right: 8px;"></i> 泥섏쓬?쇰줈 ?뚯븘媛湲?
                    </button>
                </div>
            `;
            ...
            kioskHtml = `
                <div style="text-align: center; margin-bottom: 1.5rem;">
                    ...
                    <h2 style="font-size: 1.7rem; font-weight: 800; margin-bottom: 8px;">${selectedStudent.name} ?먯깮??/h2>
                    <p style="color: var(--text-muted); font-size: 0.95rem;">?먰븯?쒕뒗 異쒓껐 ?곹깭瑜??곗튂??二쇱꽭??</p>
                </div>
                ...
                <div class="kiosk-status-container">
                    <div class="kiosk-status-card status-in" id="kiosk-action-checkin">
                        ...
                        <span class="kiosk-status-title">?깆썝 (異쒖꽍)</span>
                        <span class="kiosk-status-desc">?숈썝???꾩갑?덉뒿?덈떎.</span>
                    </div>
                    <div class="kiosk-status-card status-out" id="kiosk-action-checkout">
                        <i class="fa-solid fa-door-closed"></i>
                        <span class="kiosk-status-title">?섏썝</span>
                        <span class="kiosk-status-desc">?섏뾽 ??洹媛?⑸땲??</span>
                    </div>
                </div>

                <div style="text-align: center; margin-top: 1.5rem;">
                    <button class="btn btn-none" id="kiosk-back-to-student" style="padding: 10px 24px; font-weight: 600; color: var(--text-muted);">
                        <i class="fa-solid fa-arrow-left" style="margin-right: 8px;"></i> ?댁쟾 ?④퀎濡?
                    </button>
                </div>
            `;
        } else if (activeStep === 'complete') {
            const isCheckIn = completeStatus === 'in';
            const actionLabel = isCheckIn ? '?깆썝' : '?섏썝';
            const actionDesc = isCheckIn ? '?숈썝???덉쟾?섍쾶 ?깆썝?섏??듬땲??' : '?섏뾽??留덉튂怨??덉쟾?섍쾶 ?섏썝?섏??듬땲??';
            ...
            kioskHtml = `
                <div style="text-align: center; max-width: 480px; margin: 0 auto;">
                    ...
                    <h2 style="font-size: 1.8rem; font-weight: 800; margin-bottom: 10px; color: var(--text-main);">${selectedStudent.name} ??/h2>
                    <p style="font-size: 1.1rem; font-weight: 600; color: var(--text-main); margin-bottom: 6px;">${actionLabel}???꾨즺?섏뿀?듬땲??</p>
